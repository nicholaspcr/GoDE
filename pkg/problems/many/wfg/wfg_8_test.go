...
func TestWFG8FN(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			x: []float64{0.24199364597771478, 0.06294085809752699,
				...
				0.8210562785104756},
			expected: []float64{0.45397981, 0.85660284, 6.39589598},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.ProblemName, func(t *testing.T) {
			...
			err := Wfg8().Evaluate(&e, len(tt.expected))

			if err != nil {
				t.Errorf("failed to run the WFG8 func")
			}
			...
			if received != want {
				t.Errorf(
					"WFG8 wrong objs. received %v, expected %v",
					received,
					want,
				)
			}

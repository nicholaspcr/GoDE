...
func TestWFG5FN(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expected: []float64{0.39267686, 1.47273458, 5.71739805},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.ProblemName, func(t *testing.T) {
			...
			err := Wfg5().Evaluate(e, len(tt.expected))
			if err != nil {
				t.Errorf("failed to run the WFG5 func")
			}
			...
			if received != want {
				t.Errorf(
					"WFG5 wrong objs. received %v, expected %v",
					received,
					want,
				)
			}
